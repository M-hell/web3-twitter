...
contract Twitter is Ownable {
    ...
    function getAllTweetsFromAllOwners() external view returns (Tweet[] memory) {
        address[] memory owners = profileContract.getAllOwners();

        uint totalTweets = 0;
        for (uint i = 0; i < owners.length; i++) {
            totalTweets += tweets[owners[i]].length;
        }
        ...
        for (uint i = 0; i < owners.length; i++) {
            Tweet[] memory userTweets = tweets[owners[i]];
            for (uint j = 0; j < userTweets.length; j++) {
                allTweets[index] = userTweets[j];
                index++;
            }
        }
